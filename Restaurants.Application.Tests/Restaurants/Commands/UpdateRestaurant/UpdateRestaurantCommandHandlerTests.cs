...
public class UpdateRestaurantCommandHandlerTests
{
    ...
    public async Task Handle_WithValidRequest_ShouldUpdateRestaurants()
    {
        ...
        _restaurantsRepositoryMock.Setup(r => r.GetByIdAync(restaurantId))
            .ReturnsAsync(restaurant);

        _restaurantauthorizationServiceMock.Setup(a => a.Authorize(restaurant, Domain.Constants.ResourceOperation.Update)).Returns(true);
    ...
    public async Task Handle_WithNonExistingRestaurant_ShouldThrowNotFoundException()
    {
        ...
        _restaurantsRepositoryMock.Setup(r => r.GetByIdAync(restaurantId)).ReturnsAsync((Restaurant?)null);
    ...
    public async Task Handle_WithUnauthorizedUser_ShouldThrowForbidException()
    {
        ...
        _restaurantsRepositoryMock.Setup(r => r.GetByIdAync(restaurantId)).ReturnsAsync(existingRestaurant);

        _restaurantauthorizationServiceMock.Setup(a => a.Authorize(existingRestaurant, Domain.Constants.ResourceOperation.Update)).Returns(false);
